...
| `NEXT_PUBLIC_SITE_URL` | web 앱 (빌드 시 노출) | `https://burn-emotion.net` — canonical·OG·sitemap·robots·JSON-LD 도메인 |
| `PORT` | ws 서버 | 리스닝 포트 |
...
- **WS 서버 단일 인스턴스** — 재시작 시 presence·연결이 초기화됨. 클라이언트 자동
  재연결로 회복. 트래픽 규모상 허용.
- **빌드 시 노출 변수** — `NEXT_PUBLIC_WS_URL`·`NEXT_PUBLIC_SITE_URL` 은 빌드 타임에
  박히므로 Coolify 빌드 환경변수(build-arg)로 등록해야 함. 런타임 env 로는 교정 불가.
  특히 `NEXT_PUBLIC_SITE_URL` 누락 시 fallback 도메인이 canonical·sitemap·robots 에
  박혀 색인이 깨짐(sitemap/robots 는 정적 prerender 라 재빌드 없이는 못 고침).
