...
- [ ] **Step 1: layout.tsx 작성 — KR `<html lang="ko">` + 폰트 link**
...
- [ ] **Step 2: globals.css 교체 — 디자인 번들의 styles-v2.css에서 디자인 토큰/글로벌만 추출**
...
- [ ] **Step 3: SSR 안전 확인 — `npm run dev` 후 첫 화면이 검은 배경(`--night-0`)으로 변경되는지 확인**
...
- [ ] **Step 4: package.json scripts 추가 — `test`, `test:watch`**
...
- [ ] **Step 5: 첫 테스트 동작 확인 — 빈 더미 테스트 추가**
...
- [ ] **Step 3: 구현 — 디자인 번들 `data.jsx` 의 makeNickname을 TS로 포팅**
...
- [ ] **Step 3: 구현 — design bundle의 layoutPos를 TS 함수로 포팅**
...
- [ ] **Step 1: 모듈 CSS — 스테이지/모닥불/실루엣/입력바/피드 레이아웃**
...
- [ ] **Step 2: BonfireScene 첫 버전 작성 — 정적 더미 상태로 모든 시각 요소 렌더**
...
- [ ] **Step 1: page.tsx에서 BonfireScene을 동적 import + ssr:false 로 마운트 후 로드**
